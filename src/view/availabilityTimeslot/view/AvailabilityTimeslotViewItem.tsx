...
function AvailabilityTimeslotViewItem(props) {
  ...
  const displayableRecord = (record) => {
      ...
      return (
        <div key={record.id}>
          <Link
            ...
            to={`/availability-timeslot/${record.id}`}
          >
            {record.dayOfTheWeek}
          </Link>
        </div>
    ...
    return <div key={record.id}>{record.dayOfTheWeek}</div>;
  };
...
AvailabilityTimeslotViewItem.propTypes = {
  label: PropTypes.string,
  value: PropTypes.any,
};

export default AvailabilityTimeslotViewItem;
